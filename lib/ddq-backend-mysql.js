...
module.exports = (configValidation, crypto, EventEmitter, mysql, timers) => {
     ...
     * @param {Object} params
     */
    function trySendMessage(ddqBackendInstance, params) {
        if (cycleCounter < ddqBackendInstance.config.createMessageCycleLimit) {
            ...
            ddqBackendInstance.connection.query(`INSERT INTO ?? (hash, messageBase64, topics)
                ...
                [
                    ...
                    if (err && err.code === "ER_DUP_ENTRY") {
                        writeRecord(ddqBackendInstance, params);
                    } else if (err) {
                        ddqBackendInstance.emit(EMIT_ERR, err);
                    } else {
                        params.callback();
            ...
            params.callback(new Error("Could not send message."));
        }
    }
     ...
     * @param {Object} params
     */
    writeRecord = function (ddqBackendInstance, params) {
        ddqBackendInstance.connection.query(`UPDATE ??
            SET requeued = true
            WHERE hash = ?
                AND isProcessing = true;`,
    ...
    class DdqBackendMySql extends EventEmitter {
        ...
        /**
         * Sets up the parameters for trySendMessage and writeRecord and calls
         * the former.
         *
