...
module.exports = (config, configValidation, crypto, EventEmitter, mysql, timers) => {
    ...
    function heartbeat(ddqBackendInstance, recordId, callback) {
        ddqBackendInstance.connection.query(`USE ??
            UPDATE ??
            SET heartbeatDate = NOW()
            WHERE hash = ?
            AND isProcessing = true
            AND owner = ?;`,
            [ddqBackendInstance.config.database, ddqBackendInstance.config.table, recordId, ddqBackendInstance.owner],
            callback
        );
    }
    ...
    function poll(ddqBackendInstance, callback) {
        ...
        function findData() {
            ddqBackendInstance.connection.query(`USE ??
                ...
                ORDER BY RAND()
                LIMIT 1;`,
                [ddqBackendInstance.config.database, ddqBackendInstance.config.table, ddqBackendInstance.config.topics],
                callback
            );
    ...
    function requeue(ddqBackendInstance, recordId, callback) {
        ddqBackendInstance.connection.query(`USE ??
            ...
            WHERE hash = ?;`,
            [ddqBackendInstance.config.database, ddqBackendInstance.config.table, recordId],
            callback
        );
    }
    ...
    function restore(ddqBackendInstance) {
        ...
        function heartbeatCleanup() {
            ddqBackendInstance.connection.query(`USE ??
                ...
                AND isProcessing = true;`,
                [ddqBackendInstance.config.database, ddqBackendInstance.config.table, ddqBackendInstance.config.heartbeatLifetimeSeconds],
                (err) => {
                    if (err) {
                        console.log(err);
    ...
    class DdqBackendMySql extends EventEmitter {
        ...
        checkNow() {
            ...
            this.connection.query(`USE ??
                ...
                ORDER BY RAND()
                LIMIT 1;`,
                [this.config.database, this.config.table, this.config.topics],
                (err, record) => {
                    if (err) {
                        console.log(err.sql);
        ...
        sendMessage(message, topic, callback) {
            ...
            function trySendMessage() {
                if (cycleCounter < ddqBackendInstance.config.createMessageCycleLimitMs) {
                    cycleCounter += 1;
                    ddqBackendInstance.connection.query(`USE ??
                        INSERT INTO ?? (hash, messageBase64, topic)
                        VALUES (?, ?, ?);`,
                        [ddqBackendInstance.config.database, ddqBackendInstance.config.table, ddqBackendInstance.config.table, hash, message, topic],
                        (err) => {
            ...
            writeRecord = function () {
                ddqBackendInstance.connection.query(`USE ??
                    ...
                    WHERE hash = ?
                    AND isProcessing = true;`,
                    [ddqBackendInstance.config.database, ddqBackendInstance.config.table, ddqBackendInstance.config.table, hash],
                    (err, data) => {
                        if (err) {
                            ddqBackendInstance.emit(EMIT_ERR, err);
                        } else if (data.affectedRows) {
                            console.log("Could not update record because it does not exist.");
