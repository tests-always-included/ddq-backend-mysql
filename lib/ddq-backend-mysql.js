...
module.exports = (configValidation, crypto, debug, EventEmitter, mysql, timers) => {
    var setRequeued;
    ...
    function heartbeat(ddqBackendInstance, recordId, callback) {
        debug(`Starting heartbeat query owner: ${ddqBackendInstance.config.owner}`);
        ddqBackendInstance.connection.query(`
            ...
            [
                ...
                debug(`Completed heartbeat query owner: ${ddqBackendInstance.config.owner}`);
                callback(err);
            }
        );
    }
    ...
    function requeue(ddqBackendInstance, recordId, callback) {
         ...
         */
        debug(`Starting requeue query owner: ${ddqBackendInstance.config.owner}.`);
        ddqBackendInstance.connection.query(`
            UPDATE ??
            SET owner = null,
                isProcessing = false,
            ...
            [
                ...
                debug(`Complete requeue query owner: ${ddqBackendInstance.config.owner}.`);
                callback(err);
            }
        );
    }
    ...
    function remove(ddqBackendInstance, recordId, callback) {
         ...
         */
        debug(`Starting remove query owner: ${ddqBackendInstance.config.owner}.`);
        ddqBackendInstance.connection.query(`
            DELETE
            FROM ??
            WHERE hash = ?
            ...
            [
                ...
                // We want to make sure to requeue any message that needs to be
                // processed again. In this case if a message fails to remove
                // or requeued is set to true. Which happens when a duplicate
                // message is sent while another message is processing.
                if (err || stats.affectedRows === 0) {
                    debug(`Remove was not successful, requeue initiated owner: ${ddqBackendInstance.config.owner}`);
                    requeue(ddqBackendInstance, recordId, callback);
                } else {
                    callback();
                }

                debug(`Completed remove query owner: ${ddqBackendInstance.config.owner}.`);
            }
        );
    }
    ...
    function poll(ddqBackendInstance) {
        ...
        function checkNow() {
            ...
            debug(`Polling owner: ${ddqBackendInstance.config.owner}.`);
                    ...
                    [
                        ...
                        if (selErr) {
                            if (selErr.code === "ER_LOCK_DEADLOCK" && ddqBackendInstance.deadlockCount < ddqBackendInstance.config.deadlockCountLimit) {
                                debug(`Deadlock occurred on polling select owner: ${ddqBackendInstance.config.owner}`);
                                ddqBackendInstance.deadlockCount += 1;
                            } else {
                                debug(`Error occurred on polling select owner: ${ddqBackendInstance.config.owner}`);
                                ddqBackendInstance.emit(EMIT_ERR, selErr);
                            }
                        } else if (record.length > 0) {
                            ddqBackendInstance.deadlockCount = 0;
                            debug(`Select for update complete owner: ${ddqBackendInstance.config.owner}`);
                            ...
                            ddqBackendInstance.connection.query(`
                                ...
                                [
                                    ...
                                    if (updateErr) {
                                        debug(`Polling update error rolling back transaction owner: ${ddqBackendInstance.config.owner}`);
                                        ddqBackendInstance.connection.rollback(() => {
                                            ddqBackendInstance.emit(EMIT_ERR, updateErr);
                                        });
                                    } else {
                                        debug(`Polling transaction completed committing owner: ${ddqBackendInstance.config.owner}`);
                                        ddqBackendInstance.connection.commit((commitErr) => {
                                            if (commitErr) {
                                                ddqBackendInstance.connection.rollback(() => {
                                                    debug(`Polling commit error rolling back transaction owner: ${ddqBackendInstance.config.owner}`);
                                                 ...
                                                 * easy to use.
                                                 */
                                                debug(`Message retreived ${record[0].message} owner: ${ddqBackendInstance.config.owner}`);
                                                wrappedMessage = {
    ...
    function restore(ddqBackendInstance) {
        ...
        function heartbeatCleanup() {
            ...
            debug(`Initiating restore owner: ${ddqBackendInstance.config.owner}`);
                ...
                [
                    ...
                    if (err) {
                        if (err.code === "ER_LOCK_DEADLOCK" && ddqBackendInstance.deadlockCount < ddqBackendInstance.config.deadlockCountLimit) {
                            ddqBackendInstance.deadlockCount += 1;
                            debug(`Deadlock ocurred restoring owner: ${ddqBackendInstance.config.owner}`);
                        } else {
                            debug(`Error ocurred restoring owner: ${ddqBackendInstance.config.owner}`);
                            ddqBackendInstance.emit(EMIT_ERR, err);
                        }
                    } else {
                        ddqBackendInstance.deadlockCount = 0;
