...
module.exports = (config, configValidation, crypto, EventEmitter, mysql, timers) => {
    ...
    function heartbeatCleanup(ddqBackendInstance) {
        ...
        console.log("fsadlk;fjlkjfalkjfsdaalfkdalk;jfsadlk;jfsadlkjfsadlkj;fsadlkj;fsadlkjlkjfsalfksadfsadlfsadlkalfkdjfsadlk;;jasdfl;k;jl;kajsdfl;k");
        ddqBackendInstance.connection.query(`UPDATE ??
    ...
    class DdqBackendMySql extends EventEmitter {
        ...
        /**
         * One half of the functionality that run heartbeatCleanup with the
         * heartbeatCleanupDelay.
         */
        setHeartbeat() {
            setTimeout(heartbeatCleanup(this), this.config.heartbeatCleanupDelay);
        }
        ...
        /**
         * Polls for a random record with a delay defined by the config.delayMs.
         *
         * @param {Function} callback
         */
        poll(callback) {
             ...
             */
            function findData() {
                that.connection.query(`SELECT hash
                    FROM ??
                    ...
                    ORDER BY RAND()
                    LIMIT 1;`,
                    [that.config.table, that.config.topics],
                    callback
                );

                that.poller = timers.setTimeout(findData, that.config.delayMs);
            }

            this.poller = setTimeout(findData, this.config.delayMs);
        }
    }
