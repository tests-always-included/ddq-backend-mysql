...
module.exports = (configValidation, crypto, EventEmitter, mysql, timers) => {
    ...
    function heartbeat(ddqBackendInstance, recordId, callback) {
        ddqBackendInstance.connection.query(`
            ...
            [
                ...
                recordId,
                ddqBackendInstance.config.owner
            ], callback
        );
    }
        ...
        function heartbeatCleanup() {
            ...
            ddqBackendInstance.connection.query(`
                ...
                SET isProcessing = false,
                    owner = null,
                    requeued = false
                WHERE DATE_SUB(NOW(), INTERVAL ? SECOND) > heartbeatDate
                    AND isProcessing = true`,
                [
                    ddqBackendInstance.config.table,
    ...
    /**
     * Updates a record. If there isn't an error but no records are updated
     * (indicating the record to update doesn't exist), it will call the
     * trySendMessage method to attempt to insert the record. Otherwise, it will
